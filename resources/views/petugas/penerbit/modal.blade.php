...
<div class="modal-body">
    <form class="needs-validation" novalidate action="{{ route('petugasStorePenerbit') }}"
        method="post">
        @csrf
        <div class="mb-3">
...
<div class="modal-body">
    <form class="needs-validation" novalidate action="{{ route('updatePenerbit', $data->id) }}"
        method="post">
        @csrf
        @method('PUT')
        <div class="mb-3">
            <label for="nm_penerbit" class="form-label">Nama Lembaga</label>
            <input type="text" class="form-control" required id="nm_penerbit" name="nm_penerbit" value="{{ $data->nm_penerbit }}">
            <div class="invalid-feedback">
                Kolom Nama Lembaga Wajib Diisi!
            </div>
        </div>
