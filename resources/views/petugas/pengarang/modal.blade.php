...
<div class="modal-body">
    <form class="needs-validation" novalidate action="{{ route('petugasStorePengarang') }}"
        method="post">
        @csrf
        <div class="mb-3">
...
<div class="modal-body">
    <form class="needs-validation" novalidate action="{{ route('updatePengarang', $data->id) }}"
        method="post">
        @csrf
        @method('PUT')
        <div class="mb-3">
            <label for="nm_pengarang" class="form-label">Nama Pengarang</label>
            <input type="text" class="form-control" required id="nm_pengarang" name="nm_pengarang" value="{{ $data->nm_pengarang }}">
            <div class="invalid-feedback">
                Kolom Nama Pengarang Wajib Diisi!
            </div>
        </div>
        <div class="mb-3">
            <label for="alamat" class="form-label">Alamat</label>
            <textarea name="alamat" id="alamat" rows="4" class="form-control" required>{{ $data->alamat }}</textarea>
            <div class="invalid-feedback">
                Kolom Alamat Wajib Diisi!
            </div>
        </div>
        <div class="mb-3">
            <label for="" class="form-label">Gender</label>
            <div class="form-check">
                <input {{ $data->gender == 'L' ? 'checked' : '' }} class="form-check-input" type="radio" name="gender" id="L" required value="L">
                <label class="form-check-label" for="L">Laki-Laki</label>
            </div>
            <div class="form-check">
                <input {{ $data->gender == 'P' ? 'checked' : '' }} class="form-check-input" type="radio" name="gender" id="P" required value="P">
                <label class="form-check-label" for="P">Perempuan</label>
            </div>
            <div class="invalid-feedback">
                Pilih Salah Satu Gender!
        ...
        <div class="mb-3">
            <label for="tlp" class="form-label">Telpon</label>
            <input type="text" class="form-control" required id="tlp" name="tlp" value="{{ $data->tlp }}">
            <div class="invalid-feedback">
                Kolom No Telpon Pengarang Wajib Diisi!
            </div>
        </div>
